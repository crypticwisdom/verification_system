...
from account.models import User, Service, Transaction, ServiceDetail
...
from .serializers import BusinessTransactionSerializer, BusinessRecentVerificationSerializer, BusinessVerificationServiceSerializer,\
    BusinessServiceHistorySerializer, BusinessProfileSerializer
from util.utils import validate_email, get_incoming_request_checks
from account.utils import password_checker
from util.utils import phone_number_check
from individual.serializers import LandPageServicesSerializer


# Create your views here.
class BDashboardView(APIView, CustomPagination):
    permission_classes = [IsAuthenticated, permission.IsBusiness]
    ...
    def get(self, request):
        try:
            ...
            for service_detail_id in service_detail_ids:
                ...
                _ = BusinessRecentVerificationSerializer(service_detail, many=False, context={"request": request}).data
                data_.append(_)
            data.update({"recentVerifications": data_})

            # Transaction made by this user.
            container2 = {}
            container2.update({"recentHistory": BusinessTransactionSerializer(transactions.order_by('-id')[:10],
                                                                                many=True).data})
            data.update(container2)
            return Response(api_response(message=f"Business Dashboard Data", data=data, status=True))
        except (Exception, ) as err:
            return Response(api_response(message=f"{err}", data={}, status=False), status=status.HTTP_400_BAD_REQUEST)


class BVerificationView(APIView, CustomPagination):
    permission_classes = [IsAuthenticated, permission.IsBusiness]
    ...
    def get(self, request, pk=None):
        try:
            if pk:
                ...
                data = BusinessVerificationServiceSerializer(service, many=True, context={"request": request}).data
            ...
            serialized_pagination = LandPageServicesSerializer(
                self.paginate_queryset(free_services.distinct(), request), many=True, context={'request': request, "service_type": "free"}).data
            ...
            serialized_pagination = LandPageServicesSerializer(
                self.paginate_queryset(paid_services.distinct(), request), many=True, context={'request': request, "service_type": "paid"}).data
...
class BTransactionView(APIView, CustomPagination):
    permission_classes = [IsAuthenticated, permission.IsBusiness]
    ...
    def get(self, request):
        try:
            ...
            if query:
                ...
                query_set = query_set.filter(Q(service_detail__service_type__icontains=query) |
                                             Q(service_detail__name=query) | Q(channel__name__icontains=query) |
                                             Q(amount__contains=query) | Q(document__icontains=query))
            ...
            response = BusinessTransactionSerializer(query_set, many=True).data
            if download is False:
                paginated_query = self.paginate_queryset(query_set, request=request)
                response = self.get_paginated_response(BusinessTransactionSerializer(paginated_query, many=True).data).data
            return Response(api_response(message=f"Transaction Dashboard Data", data=response, status=True))
        except (Exception, ) as err:
            return Response(api_response(message=f"{err}", data={}, status=False), status=status.HTTP_400_BAD_REQUEST)


class BGetServiceInformation(APIView, CustomPagination):
    permission_classes = [IsAuthenticated, permission.IsBusiness]
    ...
    def get(self, request, pk=None):
        try:
            if pk:
                ...
                transactions_history = Transaction.objects.filter(owner=request.user, service_detail__service=service).order_by('id')
                paginated_query = self.paginate_queryset(transactions_history, request=request)
                response = self.get_paginated_response(BusinessServiceHistorySerializer(paginated_query, many=True, context={"user": request.user}).data)
                return Response(api_response(message=f"Services Information for '{service.name}'", data=response.data, status=True))
...
class BusinessSettingsView(APIView, CustomPagination):
    permission_classes = [IsAuthenticated, permission.IsBusiness]
        ...
        try:
            ...
            if email:
                ...
                if User.objects.filter(email=email).exists():
                    return Response(api_response(message="Email already exist", status=False), status=status.HTTP_400_BAD_REQUEST)

            if phone_number:
                phone_number_success, phone_number = phone_number_check(phone_number=phone_number)

                if not phone_number_success:
                    return Response(api_response(message=phone_number, status=False), status=status.HTTP_400_BAD_REQUEST)
            ...
            if new_password_is_current_password:
                ...
                return Response(api_response(message="Passwords does not match", status=False), status=status.HTTP_400_BAD_REQUEST)
            ...
            serialized_data = BusinessProfileSerializer(user, many=False, context={"request": request}).data
    ...
    def get(self, request):
        ...
        try:
            ...
            serialized_data = BusinessProfileSerializer(request.user, many=False, context={"request": request}).data
